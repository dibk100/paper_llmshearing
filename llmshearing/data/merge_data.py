...
def load_data(data_local, data_split):
    # 변수 갯수 수정 : origin -- data_local, data_split, tokenizer_name
...
def merge_splits_with_no_bias():
    ...
    parser.add_argument("--output_dir", type=str, default='./mds_sample_redpajama/eval')
    parser.add_argument("--input_dir", type=str, default='./mds_sample_redpajama/eval')
    parser.add_argument("--output_split", type=str, default='eval_merge')
    parser.add_argument("--split_names", nargs='+', default=['arxiv','book','c4-rp','cc','github','stackexchange','wiki'])
    ...
    if args.shuffle:
        ...
        for i, (input_dir, split_name) in enumerate(zip(args.input_dir, args.split_names)):
            data = load_data(input_dir, split_name)
            if len(args.split_rows) > 0:
                lens = int(args.split_rows[i])
            else:
                lens = len(data)
