...
def save_hf_to_composer():
    """ Convert composer model to huggingface model """ 
    # 수정 : origin -- hf_model_name_or_path, output_path
    hf_model_name_or_path='meta-llama/Llama-2-7b-hf'
    output_path='./llmshearing/models/Llama-2-7b-composer/state_dict.pt'
    
    model = AutoModelForCausalLM.from_pretrained(hf_model_name_or_path, token=mytoken)
